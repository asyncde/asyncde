...
int cde_example(asyncde::Functor1D &fitnessfunctor, unsigned int dim,
  ...
  adesettings.SetStrategy("DE/rand/rand/1/bin");
  ...
  asyncde::CDEIterator *iterator = new asyncde::CDEIterator(
...
#ifdef _GPERF_PROFILER
  ProfilerStart("cde_example.prof");
#endif

  int retvalue = cde_example(fitnessfunctor, nvars, vtr, maxfunevals, pop_size);
#ifdef _GPERF_PROFILER
  ProfilerStop();
#endif
